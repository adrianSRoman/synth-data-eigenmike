...
for height in range(0, 7): # decrease heights
    ...
    for num in outter_trayectory_top:
        ...
        x_y_z = [x, y, z]
        doa_xyz.append(x_y_z)
        distances = np.sqrt(np.sum((np.array(mic_xyz)-np.array(x_y_z))**2)) # l2
        ...
        # y_coords.append(y)
        # z_coords.append(z)
    rirdata_dict[room]['doa_xyz'].append(doa_xyz)
    rirdata_dict[room]['dist'].append(heights_list)
    rirdata_dict[room]['rir'].append(hir_list)

with open('{}.pkl'.format("rir_dict_em32"), 'wb') as outp:
